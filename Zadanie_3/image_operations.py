...
def hough_transform_circle_second(img, radius_range, min_votes):
    ...
    # Find circles with the highest number of votes
        arr_normalized = (accumulator[:,:,r_idx] - accumulator[:,:,r_idx].min()) / (accumulator[:,:,r_idx].max() - accumulator[:,:,r_idx].min())  # normalize array
        arr_uint8 = (arr_normalized * 255).astype(np.uint8)  # scale down array
        print(np.sort(accumulator[:,:,r_idx].flatten())[::-1][0:10])
